...
def main() -> None:
    """Load image, print info, crop a 400x400 center region and display it.

    The function handles missing files or invalid inputs with clear messages.
    """
    try:
    ...
    print("New shape after slicing: {}".format(crop_expanded.shape))
    print(crop_expanded)
    ...
    try:
        ...
        xticks = np.arange(0, crop.shape[1], 50, dtype=int)
        yticks = np.arange(0, crop.shape[0], 50, dtype=int)
        ...
        print("Warning: could not save image: {}".format(exc))
